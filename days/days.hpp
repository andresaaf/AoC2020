#ifndef DAYS_HPP_INCLUDED
#define DAYS_HPP_INCLUDED

#include <iostream>
#include <fstream>
#include <algorithm>
#include <vector>
#include <string>
#include <array>
#include <numeric>
#include <sstream>
#include <utility>
#include <set>
#include <unordered_set>
#include <map>
#include <unordered_map>
#include <list>
#include <tuple>

void day1(std::istream& in, int part);
void day2(std::istream& in, int part);
void day3(std::istream& in, int part);
void day4(std::istream& in, int part);
void day5(std::istream& in, int part);
void day6(std::istream& in, int part);
void day7(std::istream& in, int part);
void day8(std::istream& in, int part);
void day9(std::istream& in, int part);
void day10(std::istream& in, int part);
void day11(std::istream& in, int part);
void day12(std::istream& in, int part);
void day13(std::istream& in, int part);
void day14(std::istream& in, int part);
void day15(std::istream& in, int part);
void day16(std::istream& in, int part);
void day17(std::istream& in, int part);
void day17_part2(std::istream& in, int part);
void day18(std::istream& in, int part);
void day19(std::istream& in, int part);
void day20(std::istream& in, int part);
void day23(std::istream& in, int part);

#endif // DAYS_HPP_INCLUDED
